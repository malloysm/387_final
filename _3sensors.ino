...
// Define gym open/close times
  // if Arduino loses power during gym operation times data will not be saved for that day, operate off battery or backup to ensure no outages occur
  ...
  uint8_t GymClose[2] = {6,4}; //10:30pm
...
// Define statistic variables to save to EEPROM
  ...
  float EEMEM BikeAvgUseTimePerDayStore; // in secs for testing purposes
  float EEMEM TreadAvgUsesPerDayStore; // 
  float EEMEM TreadAvgUseTimePerDayStore; // in secs for testing purposes
  float EEMEM LatAvgUsesPerDayStore; // 
  float EEMEM LatAvgUseTimePerDayStore; // in secs for testing purposes
...
//     eeprom_update_block(&fzero, &LatAvgUsesPerDayStore, sizeof(fzero));
//     eeprom_update_block(&fzero, &LatAvgUseTimePerDayStore, sizeof(fzero));
...
void UpdateState(boolean beambroke, uint8_t Input[5], unsigned long t[3], float *AvgUseTimePerDayStore) {
...
void loop() {
  ...
  // Use like 'uint8_t Variable[2] = {row,column,state,total usage amount,color}'
  // Color 0 = off, 1 = Red, 2 = Yellow, 3 = Blue
  // State: 0 = not in use, 1 = between states, 2 = in use, 3 >= between states
  static uint8_t Bike[5] = {2,4,0,0,0};
  static uint8_t Tread[5] = {3,4,0,0,0};
  static uint8_t Lat[5] = {4,4,0,0,0};
  ...
  static unsigned long tLat[3] = {0,0,0};

  BeamBike = digitalRead(IRBike);
  BeamTread = digitalRead(IRTread);
  BeamLat = digitalRead(IRLat);

  UpdateState(BeamBike, Bike, tBike, &BikeAvgUseTimePerDayStore);
  UpdateState(BeamTread, Tread, tTread, &TreadAvgUseTimePerDayStore);
  UpdateState(BeamLat, Lat, tLat, &LatAvgUseTimePerDayStore);
  ...
  if ((unsigned long)(millis() - prevtime) >= savetime) {
    if (ArduinoPower == true || start == 1) {
      ...
      Serial.println("Lat");
      DoSave(Lat, tLat, &LatAvgUsesPerDayStore, &LatAvgUseTimePerDayStore);
      Serial.println();
